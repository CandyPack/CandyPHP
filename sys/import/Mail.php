...
class Mail {
  ...
  function view($v){
    ...
    if(count($exp) > 0) $v = implode('/',$exp).'/'.$v;
    $this->_arr['view'] = $v;
  ...
  function send(){
    if(!isset($this->_arr['to']) || !isset($this->_arr['view'])) return false;
    if(!file_exists('view/'.$this->_arr['view'].'.blade.php')){
    ...
    ob_start();
    include(\View::cacheView($this->_arr['view'].'.blade.php'));
